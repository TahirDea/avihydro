import logging
import math
from FileToLink import __version__
from FileToLink.bot import FileToLink
from FileToLink.server.exceptions import FIleNotFound
from FileToLink.utils.bot_utils import gen_linkx, verify_user
from FileToLink.config import Telegram
from FileToLink.utils.database import Database
from FileToLink.utils.translation import LANG, BUTTON
...
@FileToLink.on_message(filters.command('start') & filters.private)
    ...
    if usr_cmd == "/start":
        if Telegram.START_PIC:
            await message.reply_photo(
                ...
                caption=LANG.START_TEXT.format(message.from_user.mention, FileToLink.username),
                parse_mode=ParseMode.HTML,
                reply_markup=BUTTON.START_BUTTONS
            )
        else:
            await message.reply_text(
                text=LANG.START_TEXT.format(message.from_user.mention, FileToLink.username),
                parse_mode=ParseMode.HTML,
            ...
            try:
                ...
                if file_id == usr_cmd:
                    reply_markup, stream_text = await gen_linkx(m=message, _id=file_id,
                                                                name=[FileToLink.username, FileToLink.fname])
                    await message.reply_text(
                        text=stream_text,
                        parse_mode=ParseMode.HTML,
                        disable_web_page_preview=True,
...
@FileToLink.on_message(filters.private & filters.command(["about"]))
async def start(bot, message):
    ...
    if Telegram.START_PIC:
        await message.reply_photo(
            photo=Telegram.START_PIC,
            caption=LANG.ABOUT_TEXT.format(FileToLink.fname, __version__),
            parse_mode=ParseMode.HTML,
            reply_markup=BUTTON.ABOUT_BUTTONS
        )
    else:
        await message.reply_text(
            text=LANG.ABOUT_TEXT.format(FileToLink.fname, __version__),
            disable_web_page_preview=True,
            reply_markup=BUTTON.ABOUT_BUTTONS
        )

@FileToLink.on_message((filters.command('help')) & filters.private)
...
@FileToLink.on_message(filters.command('files') & filters.private)
